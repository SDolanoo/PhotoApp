...
class OdbiorcaService(
    ...
    suspend fun getAll(): List<Odbiorca> {
        return try {
            val snapshot = odbiorcyCol.get().await()
    ...
    suspend fun getById(id: String): Odbiorca? {
        return try {
            val doc = odbiorcyCol.document(id).get().await()
    ...
    suspend fun getByNip(nip: String): Odbiorca? {
        return try {
            val query = odbiorcyCol.whereEqualTo("nip", nip).limit(1).get().await()
    ...
    suspend fun insert(odbiorca: Odbiorca): String {
        val newDoc = odbiorcyCol.document()
    ...
    suspend fun update(odbiorca: Odbiorca) {
        odbiorcyCol.document(odbiorca.id).set(odbiorca).await()
    }

    suspend fun delete(odbiorca: Odbiorca) {
        odbiorcyCol.document(odbiorca.id).delete().await()
    }
}
